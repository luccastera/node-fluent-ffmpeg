...
function FfmpegCommand(args) {
  ...
  FfmpegCommand.prototype.autoOrient = function(callback) {
    var self = this;
    callback = callback || function() {};
    this.getMetadata(self.options.inputfile, function(meta, err) {
      var rotation = meta.video.rotate;
      if (rotation === 90) {
        self.options.additional.push('-vf', 'transpose=1');
        self.options.additional.push('-metadata:s:v', 'rotate=0');
      } else if (rotation === 180) {
        self.options.additional.push('-vf', 'hflip,vflip');
        self.options.additional.push('-metadata:s:v', 'rotate=0');
      } else if (rotation === 270) {
        self.options.additional.push('-vf', 'transpose=2');
        self.options.additional.push('-metadata:s:v', 'rotate=0');
      }
      callback(self);
    });
